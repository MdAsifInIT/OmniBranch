...
export class PreflightService {
  ...
  private async checkGitClean(): Promise<PreflightCheck> {
    const res = await this.runner.run({ executable: 'git', args: ['status', '--porcelain'], cwd: this.repositoryRoot });
  ...
  private checkStaleWorktrees(facts: RepositoryFacts): PreflightCheck {
    ...
    return {
        ...
        message: managed.length === 0 ? 'No stale worktrees found.' : `Found ${managed.length} managed worktrees.`,
        severity: 'warning'
    };
  }
  ...
  private async checkConfig(): Promise<PreflightCheck> {
    ...
    try {
        ...
        return {
            ...
            passed: result.valid,
            message: result.valid ? 'Workspace config is valid.' : 'Workspace config has validation errors.',
            severity: 'error'
        };
    } catch {
        return {
  ...
  private checkDefaultBranch(facts: RepositoryFacts): PreflightCheck {
    return {
        ...
        message: facts.defaultBranch !== null ? `Default branch is ${facts.defaultBranch}.` : 'Could not determine default branch.',
        severity: 'warning'
    };
  }
  ...
  private checkNodeVersion(): PreflightCheck {
    ...
    return {
        name: 'node-version',
        passed: version >= 22,
        message: version >= 22 ? `Node version ${version} is supported.` : `Node version ${version} is below required 22.`,
        severity: 'error'
    };
  }
  ...
  private async checkNoStaleLocks(): Promise<PreflightCheck> {
    try {
        ...
        return {
            ...
            message: locks.length === 0 ? 'No stale lock files found.' : `Found ${locks.length} lock files.`,
            severity: 'warning'
        };
    } catch {
        return {
...
export class CampaignSnapshotService {
  ...
  async capture(
    ...
    return {
        ...
        pendingWorkItems: workItems.filter(wi => wi.status !== 'succeeded' && wi.status !== 'failed' && wi.status !== 'canceled'),
...
export class CampaignCleanupService {
  ...
  async cleanup(
    ...
    if (confirm) {
        ...
        for (const wt of managedWorktrees) {
            await this.runner.run({ executable: 'git', args: ['worktree', 'remove', wt.path, '--force'], cwd: this.repositoryRoot });
            if (wt.branch) {
                await this.runner.run({ executable: 'git', args: ['branch', '-D', wt.branch.replace('refs/heads/', '')], cwd: this.repositoryRoot });
            }
        }
    }
  ...
  async cleanStale(facts: RepositoryFacts, confirm: boolean): Promise<CleanupResult> {
    return {
